...
export default function OfferOnboarding() {
  ...
  const load = useCallback(async () => {
    ...
    setCandidate(data);
    setOnboardingDate(
      data.onboarding?.onboardingDate
        ? data.onboarding.onboardingDate.slice(0, 10)
        : "",
    );
    setLoading(false);
  }, [id]);
  ...
  const markOfferSent = async () => {
    await axios.patch(`/candidates/${id}/offer`, {
    ...
    <Container maxWidth="sm" sx={{ py: 5 }}>
      ...
      <Paper sx={{ p: 4 }}>
        <Typography variant="h5" fontWeight={800} gutterBottom>
          Offer & Pre-Onboarding — {candidate.name}
        </Typography>
        ...
        {candidate.convertedToEmployee && (
          <Alert severity="success" sx={{ mb: 3 }}>
            This candidate has already been added to the Internal Portal as an
            employee.
          </Alert>
        )}
        ...
        <Stack spacing={2}>
          <Button
            variant={
              candidate.onboarding?.offerSentAt ? "outlined" : "contained"
            }
            onClick={markOfferSent}
            disabled={!!candidate.onboarding?.offerSentAt}
          >
            {candidate.onboarding?.offerSentAt
              ? "Offer Sent ✓"
              : "Mark Offer as Sent"}
          </Button>
          ...
          <Button
            variant={
              candidate.onboarding?.offerAcceptedAt ? "outlined" : "contained"
            }
            onClick={markOfferAccepted}
            disabled={
              !candidate.onboarding?.offerSentAt ||
              !!candidate.onboarding?.offerAcceptedAt
            }
          >
            {candidate.onboarding?.offerAcceptedAt
              ? "Offer Accepted ✓"
          ...
          <Button
            variant="outlined"
            onClick={saveOnboardingDate}
            disabled={!onboardingDate || candidate.convertedToEmployee}
          >
